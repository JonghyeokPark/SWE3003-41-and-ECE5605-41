...
I modified the source code for ECE5605-41 lab from "Ramzi-arpaciduseseau".
You can find original source code repository [Github](https://github.com/remzi-arpacidusseau/ostep-homework)
All possible bugs are mine.

In this lab, we design and implement the "Greedy" FTL.
Please fill in the skeleton (garbage_collect_greedy).
You are free to modify the source code such as create data structures, functions, and variables
except for "upkeep()", "dump()", "stat()" functions.
Please make sure not to PRINT anything on the screen (e.g., debug message).
"""
...
def garbage_collect_greedy(self):
    """
    (help): Design and implement Greedy FTL
    For a hint, you need to design 
    1) Record valid (live) page count per block 
    2) Victim selection (greedy)
    3) Erase block and keep status
...
if options.cmd_list == '':
    ...
    # (help): page write sequence
    write_list = []
    write_index = 0
    with open('input.txt') as f:
        write_list=[word for line in f for word in line.split(',')]
    write_list = [int(i) for i in write_list]

    assert(num_cmds==len(write_list))
    ...
    while len(cmd_list) < num_cmds:
        ...
        if which_cmd < percent_reads:
            ...
            # WRITE
            address = write_list[write_index]
            write_index += 1
